...
class BG_Network():
    ...
    def __init__(self, maze_shape, action_num):
        """Constructing the network according to the environment"""
    ...
    def bg_loop_fh(self, state):
        ...
        """Retrieve goal location"""
        goal = np.squeeze(np.where(self.maze=='G'))
        ...
        """
        if i == goal[0] and j == goal[1]: # Special processing when at the goal
    ...
    def policy(self, ep, state):
        ...
        if np.random.random() < self.epsilon:
            ...
            return action[max_x_index]


    def bg_loop_sh(self, episode, a_cnt, action, state, reward):
        """Retrieve location and action information"""
        s = tuple([state[0], state[1]]) # Update basal ganglia
    ...
    def memorize(self, ep, a_cnt, action, state, reward):
        ...
        sites = ["x", "D1", "D2", "lc_gpe", "gl_gpe", "lc_hyper", "gl_hyper", "SNr", "xs", "strio", "DA", "wstn", "wd1", "wd2"]
    ...
    def writer_csv(self,n):
        with open("sampleD-{}_test.csv".format((n)), "w", newline="") as f:
            fieldnames = ['episode', 'a_cnt', 'action', 'row', 'col', 'reward', 'DAsum']+\
                ...
                ['wd1'+str(i)+str(j) for i in range(2,9) for j in range(2,9)]+\
                ['wd2'+str(i)+str(j) for i in range(2,9) for j in range(2,9)]
